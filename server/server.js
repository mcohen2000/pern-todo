...
app.patch('/lists/:id', async(req, res) => {
    try {
        ...
        const updatedTodo = await pool.query("UPDATE lists SET title = $1 WHERE list_id = $2 RETURNING *", [title, id]);
...
app.delete('/lists/:id', async(req, res) => {
    try {
        const {id} = req.params;
        const deletedList = await pool.query("DELETE FROM lists WHERE list_id = $1", [id]);
